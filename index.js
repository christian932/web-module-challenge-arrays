...
function addFlavor(arry){
    arry.unshift("Rainbow Sherbert")
    return arry; 
}
...
function getFlavorByIndex(arry, index1){
    return arry[index1];
  }
...
function removeFlavorByName(arry, str){
...
function filterByWord(arry, str){
    let newarry = [];
    for (let i=0; i<arry.length; i++){
    if (arry[i].includes(str)){
      newarry.push(arry[i]);}}
    return newarry;}
